...
class PortalSettingBusiness
{
    ...
    {
        $setting = self::first();
        if ($request->has('logo') && !empty($request->logo) && !validate_base64($request->logo, ['png', 'jpg', 'jpeg']) && empty($setting->log)) {
        ...
        if ($request->has('favicon') && !empty($request->favicon) && !validate_base64($request->favicon, ['x-icon', 'png']) && empty($setting->favicon)) {
