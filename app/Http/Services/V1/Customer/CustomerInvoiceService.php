...
class CustomerInvoiceService
{
    ...
    {
        $invoices = CustomerInvoice::query()->with(['agency', 'customer']);
    ...
    {
        $invoice = CustomerInvoice::where('id', $id)->where('agency_id', app('agency')->id)->where('customer_service_request_id', \auth()->id())->first();
        if (!$invoice) {
